...
# The CSV maps ETCBC lexeme strings to Strong's numbers.
# Columns we care about: 'lex' (ETCBC lexeme) and 'strong_no' (or similar).
# We build a dict: lex_string -> strong_number_string
# ---------------------------------------------------------------------------
def load_ot_strongs():
    """
    Load OT Strong's numbers from BHS-with-Strong-no-extended.csv.
    The CSV is tab-delimited.  Column 'BHSsort' is a sequential word index
    that matches BHSA word node numbers directly (BHSsort 1 = node 1).
    Returns dict: {word_node_int -> strong_number_string}
    """
...
# Based on full ETCBC encoding table:
# https://annotation.github.io/text-fabric/tf/writing/hebrew.html
# Produces SBL-style academic romanization.
# ---------------------------------------------------------------------------
def bhsa_to_translit(g_word):
    """
    Convert a BHSA g_word ASCII string to readable Latin transliteration.
    Strips accent numbers and dagesh markers, maps consonants and vowels
    to standard academic equivalents.
    """
    # Consonant map: ETCBC ASCII -> SBL-style Latin
    CONS = {
        ">": "\u02bc",   # alef -> glottal stop marker
        "B": "b",
        "G": "g",
        "D": "d",
        "H": "h",
        "W": "w",
        "Z": "z",
        "X": "\u1e25",   # het -> h with dot below
        "V": "\u1e6d",   # tet -> t with dot below
        "J": "y",         # yod
        "K": "k",
        "k": "k",         # final kaf
        "L": "l",
        ...
        "n": "n",         # final nun
        "S": "s",
        "<": "\u02bf",   # ayin -> reversed comma
        "P": "p",
        "p": "p",         # final pe
        "Y": "\u1e63",   # tsadi -> s with dot below
        "y": "\u1e63",   # final tsadi
        "Q": "q",
        "R": "r",
        "C": "\u0161",   # shin -> s with caron
        "F": "\u015b",   # sin -> s with acute
        "#": "\u0161",   # shin without dot -> use shin
        "T": "t",
    }
    # Vowel map: ETCBC ASCII -> readable vowel
    VOWELS = {
        "A":  "a",        # patah
        ":A": "\u1d43",  # hataf patah -> superscript a
        "@":  "\u0101",  # qamets -> a macron
        ":@": "\u1d52",  # hataf qamets -> superscript o
        "E":  "e",        # segol
        ":E": "\u1d49",  # hataf segol -> superscript e
        ";":  "\u00ea",  # tsere -> e circumflex
        "I":  "\u00ee",  # hiriq -> i circumflex
        "O":  "\u00f4",  # holam -> o circumflex
        "U":  "u",        # qubuts
        ":":  "\u1d49",  # sheva -> superscript e (mobile) or nothing
    }
    # Two-char vowel sequences to check first
    TWO_CHAR_VOWELS = {":A", ":@", ":E"}
    ...
    while i < n:
        ...
        # Skip accent numbers (two-digit sequences like 74, 92, etc.)
        if c.isdigit() and i + 1 < n and s[i+1].isdigit():
            i += 2
            continue

        # Skip dagesh/mapiq dot and shin/sin dots
        if c == "." :
            # check for .c or .f (shin/sin dot)
        ...
        # Skip sof pasuq (00) and paseq (05)
        if c == "0" and i + 1 < n and s[i+1] in ("0", "5"):
            i += 2
            continue

        # Two-char vowels starting with :
        if c == ":" and i + 1 < n:
            two = ":" + s[i+1]
        ...
        # Skip anything else (unknown markers, spaces within word)
        i += 1
...
# Label maps
# ---------------------------------------------------------------------------
# BHSA 2021 uses English book names with underscores for spaces.
# Map dataset ID -> display name (just replace underscores with spaces,
...
def page_head(lang, direction, title):
    return (
        ...
        "}\n"
        ".ch-hd:hover { background: #1e1e32; }\n"
        ".ch-num { color: var(--teal); font-size: 0.8rem; font-weight: bold; }\n"
        ...
        "}\n"
        ".word:hover { border-color: var(--gold); }\n"
        ...
        ".no-res { text-align: center; padding: 40px; color: var(--muted); font-size: 0.85rem; display: none; }\n"
        ...
        "<header>\n"
        "  <h1>" + title + "</h1>\n"
        "  <input id=\"q\" type=\"search\" placeholder=\"Search ref or gloss...\" oninput=\"doSearch(this.value)\">\n"
...
def page_foot(credit):
    return (
        "</main>\n"
        "<div class=\"no-res\" id=\"no-res\">No matches found.</div>\n"
        "<footer>" + credit + "</footer>\n"
        "<script>\n"
        "document.querySelectorAll('.bk-hd').forEach(function(h) {\n"
        "  h.addEventListener('click', function() { h.parentElement.classList.toggle('open'); });\n"
        "});\n"
        "document.querySelectorAll('.ch-hd').forEach(function(h) {\n"
        "  h.addEventListener('click', function() { h.parentElement.classList.toggle('open'); });\n"
        "});\n"
        "function collapseAll() {\n"
        "  document.querySelectorAll('.book,.ch').forEach(function(e) { e.classList.remove('open'); });\n"
        "}\n"
        "function expandAll() {\n"
        "  document.querySelectorAll('.book,.ch').forEach(function(e) { e.classList.add('open'); });\n"
        "}\n"
        "var _st;\n"
        "function doSearch(q) {\n"
        "  clearTimeout(_st);\n"
        "  _st = setTimeout(function() { _search(q.trim().toLowerCase()); }, 200);\n"
        "}\n"
        "function _search(q) {\n"
        "  var any = false;\n"
        "  document.querySelectorAll('.book').forEach(function(bk) {\n"
        "    var bv = false;\n"
        "    bk.querySelectorAll('.ch').forEach(function(ch) {\n"
        "      var cv = false;\n"
        "      ch.querySelectorAll('.vs').forEach(function(v) {\n"
        "        var m = !q || v.textContent.toLowerCase().indexOf(q) !== -1;\n"
        "        v.classList.toggle('hidden', !m);\n"
        "        if (m) { cv = true; }\n"
        "      });\n"
        "      ch.classList.toggle('hidden', !cv);\n"
        "      if (cv) { ch.classList.add('open'); bv = true; }\n"
        "    });\n"
        "    bk.classList.toggle('hidden', !bv);\n"
        "    if (bv) { bk.classList.add('open'); any = true; }\n"
        "  });\n"
        "  document.getElementById('no-res').style.display = (any || !q) ? 'none' : 'block';\n"
        "}\n"
        "</script>\n"
        "</body>\n"
        "</html>\n"
...
def generate_ot():
    ...
    for bi, book_id in enumerate(OT_BOOK_ORDER):
        ...
        out.append("</div></div>")  # bk-body, book
...
def generate_nt():
    ...
    for book_id in NT_BOOK_ORDER:
        ...
        # Build verse_words fallback from verse container nodes
        ...
        for ch_num in all_ch_nums:
            ...
            for vs_num in all_vs_nums:
                ...
                if not clause_nodes:
                    # Fallback: show verse words flat
